...
namespace Accounting.Application.Features.Locations.Handlers
{
    public class UpdateLocationCommandHandler
    {
        ...
        public UpdateLocationCommandHandler(IAccountingDbContext context, ILogger<UpdateLocationCommandHandler> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public async Task<Result<LocationDto>> Handle(UpdateLocationCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (request.ParentId.HasValue)
                {
                    ...
                    {
                        return Result.Failure<LocationDto>("Location cannot be its own parent");
                    }
            ...
            {
                _logger.LogError(ex, "Error updating location with ID {LocationId}", request.Id);
                return Result.Failure<LocationDto>("An error occurred while updating the location");
            }
        }
    }
}